...
public partial class uc_case_view : System.Web.UI.UserControl
{
    private int _id = 0;
    ...
    protected void Detail(int id)
    {
        ...
        if (bll.Exists(id, true))
        {
            ...
            //下一条记录
            List<ZS.Model.znCase> nextM = bll.GetModelList("isCheck=1 and ID>" + this._id + " order by ID asc");
            if (nextM.Count > 0)
            {
            ...
            //上一条记录
            List<ZS.Model.znCase> preM = bll.GetModelList("isCheck=1 and ID<" + this._id + " order by ID desc");
            if (preM.Count > 0)
            {
