...
namespace ZS.DAL
{
    ...
    public partial class znSolution
    {
        ...
        public bool Exists(int ID, bool isCheck)
        {
            ...
            strSql.Append("select count(1) from znSolution");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 Title from znSolution");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int, 4)
			};
        ...
        /// 获得名称
        /// </summary>
        public string GetShortTitle(int ID)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 ShortTitle from znSolution");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int, 4)
			};
        ...
        {
            string strSql = "select top 1 ID from znSolution where isCheck=1  order by isTop desc,SortID desc,PostTime desc,ID desc";
        ...
        /// 增加一条数据
        /// </summary>
        public bool Add(ZS.Model.znSolution model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into znSolution(");
            strSql.Append("CategoryID,Title,ShortTitle,SortID,Hits,IdxPic,SmallPic,Summary,Content,LinkUrl,isUrl,isTop,isElite,isCheck,UserName,PostTime)");
            ...
            SqlParameter[] parameters = {
					...
					new SqlParameter("@ShortTitle", SqlDbType.NVarChar,255),
					new SqlParameter("@SortID", SqlDbType.Int,4),
					new SqlParameter("@Hits", SqlDbType.Int,4),
					new SqlParameter("@IdxPic", SqlDbType.NVarChar,255),
            ...
            parameters[2].Value = model.ShortTitle;
            parameters[3].Value = model.SortID;
            parameters[4].Value = model.Hits;
            parameters[5].Value = model.IdxPic;
            ...
            parameters[12].Value = model.isElite;
            parameters[13].Value = model.isCheck;
            parameters[14].Value = model.UserName;
            parameters[15].Value = model.PostTime;
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(ZS.Model.znSolution model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update znSolution set ");
            strSql.Append("CategoryID=@CategoryID,");
            strSql.Append("Title=@Title,");
            strSql.Append("ShortTitle=@ShortTitle,");
            strSql.Append("SortID=@SortID,");
            strSql.Append("Hits=@Hits,");
            strSql.Append("IdxPic=@IdxPic,");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from znSolution ");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update znSolution set " + strValue);
            strSql.Append(" where id=" + id);
        ...
        /// 得到一个对象实体
        /// </summary>
        public ZS.Model.znSolution GetModel(int ID)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 ID,CategoryID,Title,ShortTitle,SortID,Hits,IdxPic,SmallPic,Summary,Content,LinkUrl,isUrl,isTop,isElite,isCheck,UserName,PostTime from znSolution ");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
            ...
            ZS.Model.znSolution model = new ZS.Model.znSolution();
        ...
        /// 得到一个对象实体
        /// </summary>
        public ZS.Model.znSolution DataRowToModel(DataRow row)
        {
            ZS.Model.znSolution model = new ZS.Model.znSolution();
            if (row != null)
            {
                ...
                if (row["ShortTitle"] != null && row["ShortTitle"].ToString() != "")
                {
                    model.ShortTitle = row["ShortTitle"].ToString();
                }
                ...
                if (row["IdxPic"] != null && row["IdxPic"].ToString() != "")
                {
                    model.IdxPic = row["IdxPic"].ToString();
                }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ID,CategoryID,Title,ShortTitle,SortID,Hits,IdxPic,SmallPic,Summary,Content,LinkUrl,isUrl,isTop,isElite,isCheck,UserName,PostTime ");
            strSql.Append(" from znSolution ");
            if (strWhere.Trim() != "")
            {
        ...
        public DataSet GetList(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(" ID,CategoryID,Title,ShortTitle,SortID,Hits,IdxPic,SmallPic,Summary,Content,LinkUrl,isUrl,isTop,isElite,isCheck,UserName,PostTime,CategoryTitle from znSolutionView where ID>0 ");
            if (strWhere.Trim() != "")
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ID,CategoryID,Title,ShortTitle,SortID,Hits,IdxPic,SmallPic,Summary,Content,LinkUrl,isUrl,isTop,isElite,isCheck,UserName,PostTime,CategoryTitle from znSolutionView where ID>0");
            if (categoryID > 0)
            {
