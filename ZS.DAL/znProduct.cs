...
namespace ZS.DAL
{
    ...
    /// 数据访问类
    /// </summary>
    public partial class znProduct
    {
        public znProduct()
        { }
        ...
        public bool Exists(int ID, bool isCheck)
        {
            ...
            strSql.Append("select count(1) from znProduct");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 Title from znProduct");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int, 4)
			};
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 ShortTitle from znProduct");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int, 4)
			};
        ...
        {
            string strSql = "select top 1 ID from znProduct where isCheck=1  order by isTop asc, SortID desc, PostTime desc, ID desc";
        ...
        /// 增加一条数据
        /// </summary>
        public bool Add(ZS.Model.znProduct model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into znProduct(");
            strSql.Append("CategoryID,Title,ShortTitle,SortID,Hits,SmallPic,Summary,Content,LinkUrl,isUrl,isTop,isElite,isCheck,UserName,PostTime)");
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(ZS.Model.znProduct model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update znProduct set ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from znProduct ");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update znProduct set " + strValue);
            strSql.Append(" where id=" + id);
        ...
        /// 得到一个对象实体
        /// </summary>
        public ZS.Model.znProduct GetModel(int ID)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 ID,CategoryID,Title,ShortTitle,SortID,Hits,SmallPic,Summary,Content,LinkUrl,isUrl,isTop,isElite,isCheck,UserName,PostTime from znProduct ");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
            ...
            ZS.Model.znProduct model = new ZS.Model.znProduct();
        ...
        /// 得到一个对象实体
        /// </summary>
        public ZS.Model.znProduct DataRowToModel(DataRow row)
        {
            ZS.Model.znProduct model = new ZS.Model.znProduct();
            if (row != null)
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ID,CategoryID,Title,ShortTitle,SortID,Hits,SmallPic,Summary,Content,LinkUrl,isUrl,isTop,isElite,isCheck,UserName,PostTime ");
            strSql.Append(" from znProduct ");
            if (strWhere.Trim() != "")
            {
        ...
        public DataSet GetList(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(" ID,CategoryID,Title,ShortTitle,SortID,Hits,SmallPic,Summary,Content,LinkUrl,isUrl,isTop,isElite,isCheck,UserName,PostTime,CategoryTitle from znProductView where ID>0 ");
            if (strWhere.Trim() != "")
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ID,CategoryID,Title,ShortTitle,SortID,Hits,SmallPic,Summary,Content,LinkUrl,isUrl,isTop,isElite,isCheck,UserName,PostTime,CategoryTitle from znProductView where ID>0");
            if (categoryID > 0)
            {
        ...
        public DataSet GetPageList(int categoryID, int pageSize, int pageIndex, string strWhere, string sortFiled, out int recordCount)
        {
            ...
            baseSql.Append("select ID,CategoryID,Title,ShortTitle,SortID,Hits,SmallPic,Summary,Content,LinkUrl,isUrl,isTop,isElite,isCheck,UserName,PostTime,CategoryTitle from znProductView where ID>0");
            if (categoryID > 0)
            {
