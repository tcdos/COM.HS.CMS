...
namespace ZS.DAL
{
    ...
    public partial class znDownload
    {
        ...
        public bool Exists(int ID, bool isCheck)
        {
            ...
            strSql.Append("select count(1) from znDownload");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 Title from znDownload");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
                    new SqlParameter("@ID", SqlDbType.Int, 4)
            };
        ...
        {
            string strSql = "select top 1 ID from znDown where isCheck=1  order by isTop desc,SortID desc,PostTime desc,ID desc";
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into znDownload(");
            strSql.Append("CategoryID,Title,Source,SortID,Hits,SmallPic,Summary,FileUrl,isTop,isElite,isCheck,UserName,PostTime)");
            ...
            parameters[9].Value = model.isElite;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update znDownload set ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update znDownload set " + strValue);
            strSql.Append(" where id=" + id);
        ...
        public bool Delete(int ID)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from znDownload ");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
        ...
        public ZS.Model.znDownload GetModel(int ID)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 ID,CategoryID,Title,Source,SortID,Hits,SmallPic,Summary,FileUrl,isTop,isElite,isCheck,UserName,PostTime from znDownload ");
            strSql.Append(" where ID=@ID");
            SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
            ...
            ZS.Model.znDownload model = new ZS.Model.znDownload();
        ...
        /// 得到一个对象实体
        /// </summary>
        public ZS.Model.znDownload DataRowToModel(DataRow row)
        {
            ZS.Model.znDownload model = new ZS.Model.znDownload();
            if (row != null)
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ID,CategoryID,Title,Source,SortID,Hits,SmallPic,Summary,FileUrl,isTop,isElite,isCheck,UserName,PostTime ");
            strSql.Append(" from znDownload ");
            if (strWhere.Trim() != "")
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select ID,CategoryID,Title,Source,SortID,Hits,SmallPic,Summary,FileUrl,isTop,isElite,isCheck,UserName,PostTime,CategoryTitle from znDownloadView where ID>0");
            if (categoryID > 0)
            {
        ...
        public DataSet GetPageList(int categoryID, int pageSize, int pageIndex, string strWhere, string sortFiled, out int recordCount)
        {
            ...
            baseSql.Append("select ID,CategoryID,Title,Source,SortID,Hits,SmallPic,Summary,FileUrl,isTop,isElite,isCheck,UserName,PostTime,CategoryTitle from znDownloadView where ID>0");
            if (categoryID > 0)
            {
